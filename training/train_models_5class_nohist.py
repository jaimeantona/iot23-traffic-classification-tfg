"""Train and compare classification models on the five-class dataset without histograms.

The script evaluates Random Forest, LightGBM and a Multilayer Perceptron (MLP)
using only the aggregate flow features retained after removing the histogram
features. It reproduces the comparison experiment described in the TFG.
...
DEFAULT_DATASET = Path("data/dataset_balanced_100k_per_class_nohist.csv")
DEFAULT_OUTPUT_DIR = Path("results/5class_nohist")
RANDOM_STATE = 42
...
def save_importances(importances: pd.Series, output_path: Path) -> None:
    """Save feature importances sorted in descending order."""
    importances.sort_values(ascending=False).to_csv(output_path)
...
def train_random_forest(
    ...
    save_importances(importances, output_dir / "feature_importance_rf_5class_nohist.csv")

    return {
        "model": "Random Forest",
        "dataset": "no_histograms",
...
def train_lightgbm(
    ...
    importances = pd.Series(model.feature_importances_, index=x_train.columns)
    save_importances(importances, output_dir / "feature_importance_lgbm_5class_nohist.csv")
...
def train_mlp(
    ...
    loss_data.to_csv(output_dir / "mlp_loss_curve_nohist.csv", index=False)

    figure, axis = plt.subplots(figsize=(8, 5))
    axis.plot(range(1, len(model.loss_curve_) + 1), model.loss_curve_)
    axis.set_xlabel("Epoch")
    axis.set_ylabel("Loss")
    axis.set_title("MLP Training Loss Curve - Without Histograms")
    axis.grid(True, alpha=0.3)
    figure.tight_layout()
    figure.savefig(output_dir / "mlp_loss_curve_nohist.png", dpi=300)
    plt.close(figure)
...
def main() -> None:
    """Run the complete five-class experiment without histogram features."""
    args = parse_args()
    ...
    comparison.to_csv(args.output_dir / "model_comparison_5class_nohist.csv", index=False)
    ...
    print("\n=== FINAL MODEL COMPARISON - WITHOUT HISTOGRAMS ===")
