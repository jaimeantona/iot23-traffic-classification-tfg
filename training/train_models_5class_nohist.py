...
DATASET = "pipeline_hist/out/dataset_balanced_100k_per_class_nohist.csv"
...
def train_random_forest(X_train, y_train, X_test, y_test):
    ...
    importances.to_csv("feature_importance_rf_5class_nohist.csv")
...
def train_lightgbm_fast(X_train, X_val, y_train, y_val, X_test, y_test):
    ...
    importances.to_csv("feature_importance_lgbm_5class_nohist.csv")
...
def train_mlp(X_train, y_train, X_test, y_test):
    ...
    loss_df.to_csv("mlp_loss_curve_nohist.csv", index=False)

    plt.figure(figsize=(8, 5))
    plt.plot(range(1, len(mlp.loss_curve_) + 1), mlp.loss_curve_)
    plt.xlabel("Epoch")
    plt.ylabel("Loss")
    plt.title("MLP Training Loss Curve - No Histograms")
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig("mlp_loss_curve_nohist.png", dpi=300)
    plt.close()
...
def main():
    print("Loading dataset without histograms...")
    X, y = load_data()
    ...
    results_df.to_csv("model_comparison_5class_nohist.csv", index=False)

    print("\n=== FINAL MODEL COMPARISON - NO HISTOGRAMS ===")
    print(results_df[[
        "model",
        "cv_accuracy_mean",
        "cv_f1_macro_mean",
